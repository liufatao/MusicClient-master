...
public class CoverLoader {
    ...
    public Bitmap loadThumb(LatestMusic latestMusic) {
        return loadCoverLatest(latestMusic, Type.THUMB);
    }
    ...
    private Bitmap loadCoverLatest(LatestMusic music, Type type) {
        Bitmap bitmap;
        String key = getKey(music);
        LruCache<String, Bitmap> cache = cacheMap.get(type);
        if (TextUtils.isEmpty(key)) {
        ...
        return loadCoverLatest(null, type);
    }
    ...
    private String getKey(LatestMusic music) {
        if (music == null) {
            return null;
        }
    ...
    private Bitmap getDefaultCover(Type type) {
        switch (type) {
            case ROUND:
                Bitmap bitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.bg_play_page_default_cover);
                bitmap = ImageUtils.resizeImage(bitmap, roundLength, roundLength);
                return bitmap;
            case BLUR:
                return BitmapFactory.decodeResource(context.getResources(), R.drawable.bg_play_page_default);
            default:
    ...
    private Bitmap loadCoverByType(LatestMusic music, Type type) {
        Bitmap bitmap;
        if (music.getType() == Music.Type.LOCAL) {
            bitmap = loadCoverFromMediaStore(music.getAlbumId());
        } else {
