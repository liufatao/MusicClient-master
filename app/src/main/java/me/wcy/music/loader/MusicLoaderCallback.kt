...
import me.wcy.music.utils.MusicUtils.replaseUnKnowe
...
class MusicLoaderCallback(private val context: Context, private val callback: ValueCallback<List<Music>>) : LoaderManager.LoaderCallbacks<Cursor> {
    ...
    override fun onLoadFinished(loader: Loader<Cursor>?, data: Cursor?) {
        ...
        while (data.moveToNext()) {
            ...
            var album = data.getString(data.getColumnIndex(MediaStore.Audio.AudioColumns.ALBUM))
            val albumId = data.getLong(data.getColumnIndex(MediaStore.Audio.AudioColumns.ALBUM_ID))
            var path = data.getString(data.getColumnIndex(MediaStore.Audio.AudioColumns.DATA))
            val fileName = data.getString(data.getColumnIndex(MediaStore.Audio.AudioColumns.DISPLAY_NAME))
            val file = File(path)
            val parentPath = file.getParentFile().getPath()
            ...
            music.parentPath=parentPath;
            music.firstLetter= ChineseToEnglish.StringToPinyinSpecial(title).toUpperCase();
            if (++counter <= 20) {
                // 只加载前20首的缩略图
