 ...
 * 最近播放音乐数据适配器
 */
public class LatestMusicRecyclerviewAdapter extends RecyclerView.Adapter<LatestMusicRecyclerviewAdapter.ViewHolder> {
    private static final String TAG = "LatestMusicRecyclerviewAdapter";
    private List<LatestMusic> musicList;
    private Context context;
    private OnItemClickListener onItemClickListener;
    private OnMoreClickListener onMoreClickListener;
    public LatestMusicRecyclerviewAdapter(Context context, List<LatestMusic> musicList) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new ViewHolder(LayoutInflater.from(context).inflate(R.layout.item_latest_music, parent, false));
    }

    @Override
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        holder.vPlaying.setVisibility((position == AudioPlayer.get().getPlayPosition()) ? View.VISIBLE : View.INVISIBLE);
        LatestMusic latestMusic = musicList.get(position);
        Bitmap cover = CoverLoader.get().loadThumb(latestMusic);
